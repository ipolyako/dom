        ...
        string accountId, OrderReplace replacement, string token, CancellationToken ct)
    {
        ...
        if (orig.TryGetProperty("orderLegCollection", out var legsEl) && legsEl.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (var leg in legsEl.EnumerateArray())
            {
                ...
                // Schwab returns quantity as a JSON number that may be "500" or "500.0".
                // GetInt32() throws on the decimal form; GetDecimal() handles both.
                int qty;
                if (replacement.NewQuantity.HasValue)
                    qty = replacement.NewQuantity.Value;
                else if (leg.TryGetProperty("quantity", out var qEl) && qEl.ValueKind == JsonValueKind.Number)
                    qty = (int)qEl.GetDecimal();
                else
                    qty = 0;
                newLeg["quantity"] = qty;
